            ...
            <nav :class="`${prefixCls}-nav`">
                <ul>
                    <li v-for="(tab,index) in tabData"
                        :key="index">
                        <button v-if="tabCanShow(tab.key)"
                                @click="onTabHandleClick(tab.key)"
                ...
                <ul>
                    <li v-for="(tool,index) in toolTabData"
                        :key="index">
                        <button @click="onTabHandleClick(tool.key)"
                                :class="{active:tool.key === tabActiveKey}">
                            <Icon :type="tool.icon" /> {{tool.label}}
                        </button>
                    </li>
                </ul>
            </nav>
...
export default class PhotoGallery extends Vue {
     ...
     * Tab点击事件
     */
    private onTabHandleClick(tabKey: Tab): void {
    ...
    private show(): Tab {
        this.isShow = true
        this.tabActiveKey = this.showTabKey[0]
